...
export const TechTree: React.FC = () => {
  return (
    <div className="relative w-full h-[300px] flex items-center justify-center bg-slate-100 dark:bg-slate-900/50 rounded-2xl border border-[var(--border-color)] overflow-hidden">
      ...
      <svg className="w-full h-full" viewBox="0 0 400 300">
        ...
        {[
          ...
          <g key={i}>
            <circle cx={node.x} cy={node.y} r="6" fill="var(--bg-card)" stroke="#06b6d4" strokeWidth="2" />
            <text x={node.x} y={node.y - 12} textAnchor="middle" fill="#64748b" className="text-[8px]">{node.t}</text>
          </g>
        ))}
      </svg>
...
export const ScholarRelationshipTree: React.FC<{ scholarId: string }> = ({ scholarId }) => {
  return (
    <div className="relative w-full h-[400px] glass rounded-3xl overflow-hidden flex items-center justify-center">
      <div className="absolute top-4 left-6 text-cyan-600 dark:text-cyan-400 text-sm font-bold uppercase tracking-widest">Collaborator Network</div>
      <svg className="w-full h-full" viewBox="0 0 500 400">
        ...
        {/* Connection Lines */}
        <g stroke="var(--border-color)" strokeWidth="1">
        ...
        <text x="250" y="240" textAnchor="middle" fill="var(--text-base)" className="text-xs font-bold">Primary Target</text>
        ...
        {[
          ...
          <g key={i}>
            <circle cx={node.x} cy={node.y} r="12" fill="var(--bg-card)" stroke="#8b5cf6" strokeWidth="2" />
            <text x={node.x} y={node.y + 25} textAnchor="middle" fill="#64748b" className="text-[10px]">{node.n}</text>
          </g>
        ))}
      </svg>
    </div>
