...
import { SCHOLARS_MOCK } from '../constants';
...
const ScholarTrendCurve: React.FC<{ data: { year: number; value: number }[]; onClick?: () => void }> = ({ data, onClick }) => (
  ...
  >
    ...
    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/trend:opacity-100 transition-opacity pointer-events-none">
       <span className="text-[8px] font-black text-cyan-400 uppercase bg-slate-950/80 px-2 py-1 rounded border border-cyan-500/30">View Big Chart</span>
    </div>
  </div>
);

const TimelineHover: React.FC<{ items: any[]; title: string; labelKey: string }> = ({ items, title, labelKey }) => (
  <div className="absolute inset-0 bg-slate-950/98 backdrop-blur-md p-6 opacity-0 group-hover:opacity-100 transition-all duration-300 z-50 flex flex-col justify-center border border-white/10 rounded-3xl pointer-events-none overflow-hidden">
    <h5 className="text-[10px] text-cyan-400 font-bold uppercase tracking-[0.3em] mb-4">{title} Trajectory</h5>
    <div className="space-y-4 relative border-l border-white/10 ml-2 pl-6 overflow-y-auto custom-scrollbar max-h-full">
      {items.map((item, i) => (
        <div key={i} className="relative">
          ...
          <div className="text-[10px] text-slate-400 font-medium">{item[labelKey] || item.name || item.institution}</div>
        </div>
      ))}
    </div>
  </div>
...
const MentorshipTree: React.FC<{ scholar: Scholar }> = ({ scholar }) => (
  <div className="absolute inset-0 bg-slate-950/98 backdrop-blur-md p-6 opacity-0 group-hover:opacity-100 transition-all duration-300 z-50 flex flex-col justify-center border border-white/10 rounded-3xl">
    <div className="grid grid-cols-2 h-full">
      <div className="border-r border-white/10 pr-4 overflow-hidden">
        <h5 className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest mb-4">Masters (Mentors)</h5>
        <div className="space-y-3 overflow-y-auto custom-scrollbar max-h-[80%]">
          {scholar.teachers.length > 0 ? scholar.teachers.map(t => (
            <div key={t.id} className="flex items-center gap-3 p-2 glass rounded-xl border-slate-800">
              <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center font-bold text-[10px] shrink-0">{t.avatar}</div>
      ...
      <div className="pl-4 overflow-hidden">
        <h5 className="text-[10px] text-purple-400 font-bold uppercase tracking-widest mb-4">Protégés (Students)</h5>
        <div className="space-y-3 overflow-y-auto custom-scrollbar max-h-[80%]">
          {scholar.students.length > 0 ? scholar.students.map(s => (
            <div key={s.id} className="flex items-center gap-3 p-2 glass rounded-xl border-slate-800">
              <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center font-bold text-[10px] shrink-0">{s.avatar}</div>
...
const ScholarCard: React.FC<{ scholar: Scholar; onTrendClick: (scholar: Scholar) => void }> = ({ scholar, onTrendClick }) => {
  ...
  return (
    <div className="glass p-6 rounded-[32px] border border-slate-800 hover:border-cyan-500/30 transition-all relative group h-full">
      <div className="flex gap-6 h-full">
        {/* Left Side: Identity */}
        <div className="flex-1 space-y-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-3xl bg-slate-900 border border-slate-800 flex items-center justify-center text-xl font-black text-cyan-400 relative overflow-hidden shrink-0">
               {scholar.avatar}
               <div className="absolute inset-0 bg-cyan-500/5 animate-pulse"></div>
            </div>
            <div className="min-w-0">
              <h4 className="text-xl font-black text-white truncate">{scholar.nameEn}</h4>
          ...
          <div className="space-y-2">
            <div 
              className="flex items-center gap-2 text-xs text-slate-400 group/link cursor-help"
            ...
            >
              ...
              <span className="font-medium group-hover/link:text-cyan-400 transition-colors truncate">{scholar.institution.join(' • ')}</span>
            </div>
            <div 
              className="flex items-center gap-2 text-xs text-slate-400 group/link cursor-help"
            ...
            >
              ...
              <span className="font-medium group-hover/link:text-yellow-400 transition-colors truncate">{scholar.awards[0]?.name || 'N/A'}</span>
            </div>
          </div>
          ...
          <div className="pt-2">
            <div className="flex justify-between items-center mb-2">
              ...
              <button 
                ...
                className="text-[9px] text-cyan-400 hover:text-white font-black uppercase transition-all px-2 py-1 bg-cyan-500/5 rounded-md border border-cyan-500/10 hover:border-cyan-400/50"
              >
        ...
        <div className="w-32 flex flex-col justify-between items-end shrink-0">
          <div className="text-right">
            <div className="text-[10px] text-slate-600 font-black uppercase mb-1">Impact Rank</div>
            <div className="text-2xl font-black text-white mono glow-text">{formatHotness(scholar.hotness)}</div>
          </div>
          ...
          <div className="space-y-2 w-full">
            <button 
              ...
              onMouseLeave={() => setActiveOverlay(null)}
              className="w-full py-2 bg-slate-900 border border-slate-800 rounded-xl text-[10px] font-black uppercase text-slate-400 hover:text-white hover:border-indigo-500/50 transition-all flex items-center justify-center gap-2"
            >
              <i className="fa-solid fa-diagram-project"></i> Gene Map
            </button>
            <div className="px-3 py-1 bg-cyan-500/10 text-cyan-400 text-[10px] font-black rounded-lg text-center border border-cyan-500/20 truncate">
               {scholar.citations.toLocaleString()} Citations
            </div>
          </div>
        </div>
      </div>
...
const TrendDetailModal: React.FC<{ scholar: Scholar; onClose: () => void }> = ({ scholar, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/95 backdrop-blur-2xl p-4 lg:p-12 animate-in fade-in zoom-in duration-300">
      <div className="glass w-full max-w-6xl rounded-[48px] border border-white/10 p-8 lg:p-16 flex flex-col relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        <button 
          onClick={onClose}
          className="absolute top-8 right-8 w-12 h-12 rounded-full glass border-white/5 flex items-center justify-center text-slate-400 hover:text-white hover:border-cyan-500/50 transition-all z-[100]"
        >
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 mb-12">
          <div>
            <div className="flex items-center gap-4 mb-4">
              <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 flex items-center justify-center text-2xl font-black text-cyan-400 border border-cyan-500/20">{scholar.avatar}</div>
              <div>
                <h3 className="text-4xl font-black text-white tracking-tighter">{scholar.nameEn}</h3>
                <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px]">{scholar.institution[0]}</p>
              </div>
            </div>
            <h2 className="text-2xl font-bold text-slate-200">Trajectory Intelligence Analysis</h2>
          </div>

          <div className="flex gap-2 bg-slate-900/60 p-1.5 rounded-2xl border border-slate-800 self-center lg:self-auto">
            <button 
              onClick={() => setMetric('hotness')}
              className={`px-8 py-3 rounded-xl text-xs font-black transition-all ${metric === 'hotness' ? 'bg-cyan-500 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
            >
              HOTNESS INDEX
            </button>
            <button 
              onClick={() => setMetric('citations')}
              className={`px-8 py-3 rounded-xl text-xs font-black transition-all ${metric === 'citations' ? 'bg-indigo-500 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
            >
              CITATION GROWTH
            </button>
          </div>
        ...
        <div className="h-[450px] w-full min-h-[450px]">
           <ResponsiveContainer width="100%" height="100%">
             <AreaChart data={chartData} key={scholar.id + metric}>
                ...
                <XAxis dataKey="year" stroke="#475569" fontSize={12} axisLine={false} tickLine={false} tickMargin={10} fontWeight="bold" />
                <YAxis stroke="#475569" fontSize={12} axisLine={false} tickLine={false} tickMargin={10} fontWeight="bold" />
                <Tooltip 
        ...
        <div className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-6">
           <div className="p-6 glass rounded-3xl border-white/5">
              ...
              <div className="text-2xl font-black text-white">+{((scholar.hotness % 20) + 10).toFixed(1)}%</div>
           </div>
           <div className="p-6 glass rounded-3xl border-white/5">
              <div className="text-[10px] text-slate-500 font-black uppercase mb-2">Field Influence</div>
              <div className="text-2xl font-black text-cyan-400">Top 0.1%</div>
           </div>
           <div className="p-6 glass rounded-3xl border-white/5">
              <div className="text-[10px] text-slate-500 font-black uppercase mb-2">Collaboration Index</div>
              <div className="text-2xl font-black text-indigo-400">S-Rank</div>
           </div>
           <div className="p-6 glass rounded-3xl border-white/5">
              <div className="text-[10px] text-slate-500 font-black uppercase mb-2">Next Milestone</div>
              <div className="text-lg font-black text-white uppercase truncate">Prob 84% Expansion</div>
           </div>
        </div>
      </div>
    </div>
...
const ClusterGraph: React.FC<{ scholars: Scholar[] }> = ({ scholars }) => {
      ...
      nodesMap.slice(i + 1).forEach(other => {
        ...
        if (sharedField || sameInst) {
          linksArr.push({
            source: node,
            target: other,
            color: sharedField ? node.color : '#ffffff10'
          });
        }
      });
    });
  ...
  return (
    <div className="w-full h-full relative group/graph">
          ...
          {nodes.map(node => (
            ...
            >
              ...
              <text 
                ...
                textAnchor="middle" 
                fill={hoveredNode === node.id ? "white" : "#94a3b8"} 
      ...
      {hoveredNode && (
        <div className="absolute top-10 right-10 glass p-6 rounded-3xl border border-cyan-500/30 animate-in fade-in zoom-in duration-300 w-72 shadow-[0_0_50px_rgba(6,182,212,0.3)] z-[100]">
           {(() => {
             ...
             return (
               <div className="space-y-4">
                 <div className="flex items-center gap-4">
                   <div className="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center font-black text-cyan-400 relative overflow-hidden shrink-0">
                     {s.avatar}
                     <div className="absolute inset-0 bg-cyan-500/5 animate-pulse"></div>
                   </div>
                   <div className="min-w-0">
                     <div className="text-base font-black text-white truncate">{s.nameEn}</div>
                 ...
                 <div className="grid grid-cols-2 gap-3 pt-3 border-t border-white/5">
                   <div className="p-2 bg-slate-900/50 rounded-xl border border-slate-800">
                     ...
                     <div className="text-base font-black text-cyan-400 mono leading-none">
                       {formatHotness(s.hotness)}
                     </div>
                   </div>
                   <div className="p-2 bg-slate-900/50 rounded-xl border border-slate-800">
                     <div className="text-[8px] text-slate-500 font-black uppercase mb-1">Citations</div>
                     <div className="text-base font-black text-white mono leading-none">{(s.citations/1000).toFixed(1)}k</div>
                   </div>
                 </div>
...
export const Scholars: React.FC = () => {
  ...
  const filteredScholars = useMemo(() => {
    let list = [...SCHOLARS_MOCK];
    if (selectedRegion !== 'All Regions' && selectedRegion !== 'All') list = list.filter(s => s.region === selectedRegion);
    
    switch (activeSort) {
      case SortOption.CITATIONS: list.sort((a, b) => b.citations - a.citations); break;
      case SortOption.HOTNESS:
      case SortOption.INFLUENCE: list.sort((a, b) => b.hotness - a.hotness); break;
    }
    return list;
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-700">
      {/* Branding Header */}
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-slate-800 pb-8 gap-4 px-4">
        <div>
          <h1 className="text-5xl font-black text-white glow-text tracking-tighter">clawd bot</h1>
          <p className="text-cyan-400 font-bold uppercase tracking-[0.4em] text-[10px] mt-2">Discovery of Key AI Figures</p>
        </div>
        <div className="flex gap-6 items-center">
          <div className="text-right">
            <div className="text-[10px] text-slate-500 font-black uppercase mb-1">Intelligence SYNC</div>
            <div className="text-2xl font-black text-white mono">LV. 98</div>
          </div>
      ...
      {/* 1. Filter Matrix */}
      <div className="glass p-8 rounded-[40px] border border-slate-800 flex flex-col gap-8 mx-4">
        <div className="flex flex-wrap gap-8 items-end justify-between">
          ...
          <div className="space-y-4 flex-1 min-w-[300px]">
             ...
             <div className="flex flex-wrap items-center gap-4 bg-slate-900/40 p-4 rounded-3xl border border-slate-800 shadow-inner">
               <div className="flex items-center gap-2">
                 <select value={startYear} onChange={e => setStartYear(Number(e.target.value))} className="bg-slate-950 border border-slate-800 text-xs font-bold text-cyan-400 rounded-lg px-3 py-1.5 outline-none">
                    {years.map(y => <option key={y} value={y}>{y}Y</option>)}
                 </select>
                 <select value={startMonth} onChange={e => setStartMonth(Number(e.target.value))} className="bg-slate-950 border border-slate-800 text-xs font-bold text-cyan-400 rounded-lg px-3 py-1.5 outline-none">
                    {months.map(m => <option key={m} value={m}>{m}M</option>)}
                 </select>
               </div>
               <span className="text-slate-600 font-bold text-xs uppercase tracking-widest">To</span>
               <div className="flex items-center gap-2">
                 <select value={endYear} onChange={e => setEndYear(Number(e.target.value))} className="bg-slate-950 border border-slate-800 text-xs font-bold text-cyan-400 rounded-lg px-3 py-1.5 outline-none">
                    {years.map(y => <option key={y} value={y}>{y}Y</option>)}
                 </select>
                 <select value={endMonth} onChange={e => setEndMonth(Number(e.target.value))} className="bg-slate-950 border border-slate-800 text-xs font-bold text-cyan-400 rounded-lg px-3 py-1.5 outline-none">
                    {months.map(m => <option key={m} value={m}>{m}M</option>)}
                 </select>
               </div>
               <div className="ml-auto flex items-center gap-2 text-[10px] text-emerald-400 font-bold uppercase">
             ...
             <div className="flex gap-2 bg-slate-900/60 p-1.5 rounded-2xl border border-slate-800">
               {['INFLUENCE', 'CITATIONS', 'HOTNESS'].map(opt => (
                 <button 
                  ...
                  className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${activeSort === opt ? 'bg-cyan-500 text-white shadow-lg' : 'text-slate-500'}`}
                 >
                   {opt}
                 </button>
               ))}
          ...
          <div className="space-y-4">
             ...
             <select 
              ...
              className="bg-slate-900 border border-slate-800 rounded-xl px-6 py-3 text-xs font-bold text-white outline-none focus:border-cyan-500/50 appearance-none min-w-[150px]"
             >
      ...
      <section className="space-y-8 px-4">
        <div className="flex justify-between items-center">
          ...
          <span className="text-[10px] text-slate-500 font-bold tracking-widest bg-slate-900/50 px-3 py-1.5 rounded-full border border-slate-800">
             {filteredScholars.length} Active Nodes Detected
          </span>
        </div>
        ...
        <div className="flex justify-center pt-8">
           {showCount < filteredScholars.length ? (
             <button 
              ...
              className="px-12 py-5 glass border-slate-800 rounded-3xl text-[10px] font-black text-cyan-400 hover:bg-cyan-500/10 hover:border-cyan-500/30 transition-all uppercase tracking-[0.4em] shadow-xl group"
             >
             ...
             <button 
              ...
              className="px-12 py-5 glass border-slate-800 rounded-3xl text-[10px] font-black text-slate-500 hover:text-white transition-all uppercase tracking-[0.4em]"
             >
               Sync Collapse
             </button>
           )}
      ...
      <section className="glass p-12 rounded-[60px] border border-slate-800 h-[600px] relative overflow-hidden flex flex-col items-center justify-center mx-4">
        <div className="absolute top-10 left-10 z-10">
          <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Human Talent Clusters</h3>
        ...
        <div className="w-full h-full relative">
           ...
           <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-10">
              <i className="fa-solid fa-atom text-white text-9xl animate-spin-slow"></i>
           </div>
        </div>
      </section>
